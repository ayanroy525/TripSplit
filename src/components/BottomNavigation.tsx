...
export type NavTab = "home" | "expenses" | "settlement" | "analytics" | "people";

interface BottomNavigationProps {
  activeTab: NavTab | "activity";
...
export function BottomNavigation({
  ...
  return (
    <nav
      ...
      className="fixed bottom-0 left-0 right-0 z-40 bg-[var(--c-card,#1E293B)] border-t border-[var(--c-line,#334155)] shadow-lg pb-[env(safe-area-inset-bottom,6px)] md:hidden"
    >
      <div className="max-w-md mx-auto flex items-center justify-around h-16 px-2 relative">
        ...
        <button
          ...
          className={`flex-1 flex flex-col items-center justify-center gap-1 h-full min-h-[44px] cursor-pointer transition-colors ${
            activeTab === "home"
              ...
              : "text-[var(--c-inkSoft,#94A3B8)] font-medium"
          }`}
        >
        ...
        <div className="flex-1 flex items-center justify-center h-full">
          <button
            ...
            className="w-12 h-12 rounded-full bg-[var(--c-teal,#2DD4BF)] text-[var(--c-teal-contrast-text,#0F172A)] border-4 border-[var(--c-paper,#0F172A)] shadow-md flex items-center justify-center -translate-y-3 hover:brightness-110 active:scale-95 transition-transform cursor-pointer"
            title="Add Expense"
          >
